                ...
                for index, post in enumerate(posts):
                    ...
                    if post_id:
                        ...
                        if api_data:
                            ...
                            print(f"  Failed to fetch API data for post {post_id}, falling back to HTML scraping")
                            # Fallback: scrape available data directly from HTML
                            is_featured = await self.check_if_featured(post)
                            date_published = await self.scrape_datetime(post)
                            relative_date = await self.scrape_text(post, 'time span')
                            description = await self.scrape_description(post)
                            image_url = await self.scrape_image(post)
                            price = await self.scrape_text(post, '[class*="price"]') or await self.scrape_text(post, 'strong')
                            ...
                            card_data = {
                                ...
                                'image_url': image_url,
                                'link': f"https://www.boshamlan.com/p/{post_id}",
                                'mobile_number': None,
                                'views_number': None
                            }
